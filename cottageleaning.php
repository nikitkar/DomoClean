<?php
/*
Template Name: CottageClining
*/
?>

<?php get_header(); ?>

<main class="main">
    <section class="breadcrumbs--info">
        <div class="conteiner">
            <div class="breadcrumbsWrapper">
                <div class="breadcrumbs">
                    <a class="breadcrumbs--main" href="/">
                        На главную</a>
                    <span class="breadcrumbs--separator">/</span>
                    <a class="breadcrumbs--child" href="/cottage-cleaning">
                        Уборка коттеджа
                    </a>
                </div>
            </div>
        </div>
    </section>

    <div class="mainWrapper">
        <div class="conteiner">
            <div class="mainWrapper--inner">
                <section class="CleaningAfterRenovation">
                    <div class="CleaningAfterRenovation--info">
                        <h1 class="CleaningAfterRenovation--title">
                            Уборка коттеджа
                        </h1>
                        <p class="CleaningAfterRenovation--text">
                            Приятно, когда все свое: дом, территория рядом. Об этом мечтает буквально каждый второй житель России. Однако идиллия не так совершенна. Большой дом требует регулярной уборки, мытья полов, чистки других поверхностей. В данной ситуации задача объективно трудная.
                        </p>
                        <br>
                        <p class="CleaningAfterRenovation--text">
                            Среднестатистический коттедж характеризуется большой площадью. Она превышает аналогичный показатель даже для вместительной квартиры. Поэтому убираться в доме – задача непростая. Даже на простую поверхностную уборку может уйти день. А на генеральную...
                        </p>
                        <br>
                        <p class="CleaningAfterRenovation--text">
                            Представленная задача имеет простое решение. Речь об обращении за услугами клинеров. Мы готовы быстро помочь вам с уборкой коттеджа. Выполняем работу в минимально возможный срок. Гарантируем приемлемую и честно установленную цену.
                        </p>
                        <br>
                    </div>
                </section>
                <section class="WhyCleaning">
                    <div class="WhyCleaning--info">
                        <h2 class="WhyCleaning--title">
                            Стоимость уборки коттеджа
                        </h2>
                        <div class="WhyCleaningImg" style="background-image:url(<?php bloginfo('template_url'); ?>/assets/images/CottageClining/back.webp);">
                        </div>
                        <p class="WhyCleaning--text">
                            Каждый коттедж отличается от других. Площадью, ухоженностью. Именно от этого зависит масштаб работ и стоимость. Помимо этого наши специалисты при расчете цены уточнят пожелания заказчика, уровень загрязненности и иные факторы. Все индивидуально.
                        </p>
                        <br>
                        <p class="WhyCleaning--text">
                            Мы специализируемся не только на генеральной уборке или поверхностной, но и реализуем соответствующие мероприятия после ремонта, строительства. Эти виды деятельности клининговой компании – основные. Благодаря оказываемым нашими специалистами услугам вы можете сэкономить время, силы, энергию, направить их в требуемое русло.
                        </p>
                        <br>
                        <p class="WhyCleaning--text WhyCleaning--text--bold">
                            Оказываем клининговые услуги профессионально. Гарантируем качество. Имеем в распоряжении специальные средства и оборудование. Ваш дом в сжатые сроки станет чистым, уютным и комфортным. Находиться в нем будет исключительно приятно.
                        </p>
                        <br>
                    </div>
                </section>
                <section class="StandardServicesC">
                    <div class="StandardServicesCText">
                        <h1 class="StandardServicesC--title">
                            Что входит в уборку
                        </h1>
                        <p class="StandardServicesC--text">
                            За услугой по уборке частного дома к нам обращаются многие жители Казани. Она является достаточно востребованной. Нас спрашивают, какие именно работы выполняют клинеры компании. Отвечаем: в данный перечень входит несколько позиций.
                        </p>
                    </div>
                    <div class="StandardServicesCBlock">
                        <ul class="StandardServicesCBlock--info">
                            <li class="StandardServicesCBlock--items">
                                <div class="StandardServicesCBlock--imgInner" style="background: url(<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/decor/Img6.webp); background-position: center; background-repeat: no-repeat; background-size: contain">
                                    <img class="StandardServicesCBlock--img" src="<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/Img10.webp" alt="offerItemImg1">
                                </div>
                                <p class="StandardServicesCBlock--title">Очистка стен от пятен, загрязнений.</p>
                            </li>
                            <li class="StandardServicesCBlock--items">
                                <div class="StandardServicesCBlock--imgInner" style="background: url(<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/decor/Img6.webp); background-position: center; background-repeat: no-repeat; background-size: contain">
                                    <img class="StandardServicesCBlock--img" src="<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/Img17.webp" alt="offerItemImg1">
                                </div>
                                <p class="StandardServicesCBlock--title">Мытье полов, чистка других покрытий, а также дверей, подоконников, радиаторов и прочих запыленных поверхностей.</p>
                            </li>
                            <li class="StandardServicesCBlock--items">
                                <div class="StandardServicesCBlock--imgInner" style="background: url(<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/decor/Img6.webp); background-position: center; background-repeat: no-repeat; background-size: contain">
                                    <img class="StandardServicesCBlock--img" src="<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/Img13.webp" alt="offerItemImg1">
                                </div>
                                <p class="StandardServicesCBlock--title">Очистка санузла, туалета, ванны, иных помещений с дополнительной обработкой защищающими средствами.</p>
                            </li>
                            <li class="StandardServicesCBlock--items">
                                <div class="StandardServicesCBlock--imgInner" style="background: url(<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/decor/Img6.webp); background-position: center; background-repeat: no-repeat; background-size: contain">
                                    <img class="StandardServicesCBlock--img" src="<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/Img2.webp" alt="offerItemImg1">
                                </div>
                                <p class="StandardServicesCBlock--title">Наведение чистоты в гостиной, кухне, спальне.</p>
                            </li>
                            <li class="StandardServicesCBlock--items">
                                <div class="StandardServicesCBlock--imgInner" style="background: url(<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/decor/Img6.webp); background-position: center; background-repeat: no-repeat; background-size: contain">
                                    <img class="StandardServicesCBlock--img" src="<?php bloginfo('template_url'); ?>/assets/images/frequentlyUsedIcons/Img8.webp" alt="offerItemImg1">
                                </div>
                                <p class="StandardServicesCBlock--title">Чистка бытовой техники.</p>
                            </li>
                        </ul>
                    </div>
                </section>
                <section class="AfterRenivation">
                    <div class="AfterRenivationText">
                        <h2 class="AfterRenivationText--title">
                            Прайс-лист на уборку коттеджей в Казани и пригороде.
                        </h2>
                    </div>
                    <div class="AfterRenivationTable">
                        <table class="AfterRenivationTable--table">
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--th">Виды уборок:</td>
                                <td class="AfterRenivationTable--table--th">Ед.измерения</td>
                                <td class="AfterRenivationTable--table--th">Цена</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Уборка коттеждей</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 100-150 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Уборка после ремонта</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 130-160 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Уборка дачи</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 150 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Уборка полов</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 40 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Влажная уборка дома</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 60 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Генеральная уборка</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 100-150 руб БЕЗ ОКОН</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Мытье окон</td>
                                <td class="AfterRenivationTable--table--items">Створка</td>
                                <td class="AfterRenivationTable--table--items">от 500-800 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Клининг дома</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 100-150 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Мытье холодильника</td>
                                <td class="AfterRenivationTable--table--items">1 шт</td>
                                <td class="AfterRenivationTable--table--items">700 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Комплексная уборка</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 100-150 руб БЕЗ ОКОН</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Поддерживающая уборка(экспресс)</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 60 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Уборка частного дома</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 100-150 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Уборка нежилого помещения</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 150-170 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Послестроительная уборка</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 130-160 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Мытье духовки</td>
                                <td class="AfterRenivationTable--table--items">1 ед</td>
                                <td class="AfterRenivationTable--table--items">600 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Срочная уборка</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">по договоренности</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Мытье люстры</td>
                                <td class="AfterRenivationTable--table--items">1 ед</td>
                                <td class="AfterRenivationTable--table--items">от 300 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Мытье жалюзи</td>
                                <td class="AfterRenivationTable--table--items">1 ед</td>
                                <td class="AfterRenivationTable--table--items">200 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Мытье микроволновки</td>
                                <td class="AfterRenivationTable--table--items">1 ед</td>
                                <td class="AfterRenivationTable--table--items">400 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Уборка после мероприятия/праздника</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 130-160 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Уборка деревянных домов</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 170 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Уборка перед праздниками</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">от 60 руб</td>
                            </tr>
                            <tr class="AfterRenivationTable--table--str">
                                <td class="AfterRenivationTable--table--items">Генеральная уборка загородного дома</td>
                                <td class="AfterRenivationTable--table--items">1 м2</td>
                                <td class="AfterRenivationTable--table--items">по договоренности</td>
                            </tr>
                        </table>
                    </div>
                </section>
                <section class="commentCustomers">
                    <div class="commentCustomersWrapper">
                        <h2 class="commentCustomersTitle">Отзывы наших клиентов</h2>

                        <div class="commentCustomersContent" style="padding: 0; box-shadow: none;">
                            <div class="commentCustomersContent--wrapper">
                                <div class="commentContentTitle">
                                    <div class="commentTitle--img">
                                        <img src="<?php bloginfo('template_url'); ?>/assets/images/CliningOffice/avatar.webp" alt="commentTitleAvatar">
                                    </div>
                                    <div class="commentTitle--personInfo">
                                        <p class="commentTitle--personInfoName">Отзыв о уборке коттеджа в поселке Салмачи</p>
                                        <span class="commentTitle--personInfoWhatDoes">Клиент</span>
                                        <div class="commentTitle--personInfoEstimation">
                                            <div class="personInfoEstimationStar">
                                                <span class="star"><svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M7.64931 1.16724L9.619 5.15762L14.0237 5.80143L10.8365 8.90577L11.5887 13.2914L7.64931 11.2197L3.70993 13.2914L4.46211 8.90577L1.2749 5.80143L5.67962 5.15762L7.64931 1.16724Z" fill="var(--Color__Start)" stroke="var(--Color__Start)" stroke-width="1.27488" stroke-linecap="round" stroke-linejoin="round" />
                                                    </svg>
                                                </span>
                                                <span class="star"><svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M7.64931 1.16724L9.619 5.15762L14.0237 5.80143L10.8365 8.90577L11.5887 13.2914L7.64931 11.2197L3.70993 13.2914L4.46211 8.90577L1.2749 5.80143L5.67962 5.15762L7.64931 1.16724Z" fill="var(--Color__Start)" stroke="var(--Color__Start)" stroke-width="1.27488" stroke-linecap="round" stroke-linejoin="round" />
                                                    </svg>
                                                </span>
                                                <span class="star"><svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M7.64931 1.16724L9.619 5.15762L14.0237 5.80143L10.8365 8.90577L11.5887 13.2914L7.64931 11.2197L3.70993 13.2914L4.46211 8.90577L1.2749 5.80143L5.67962 5.15762L7.64931 1.16724Z" fill="var(--Color__Start)" stroke="var(--Color__Start)" stroke-width="1.27488" stroke-linecap="round" stroke-linejoin="round" />
                                                    </svg>
                                                </span>
                                                <span class="star"><svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M7.64931 1.16724L9.619 5.15762L14.0237 5.80143L10.8365 8.90577L11.5887 13.2914L7.64931 11.2197L3.70993 13.2914L4.46211 8.90577L1.2749 5.80143L5.67962 5.15762L7.64931 1.16724Z" fill="var(--Color__Start)" stroke="var(--Color__Start)" stroke-width="1.27488" stroke-linecap="round" stroke-linejoin="round" />
                                                    </svg>
                                                </span>
                                                <span class="star"><svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M7.64931 1.16724L9.619 5.15762L14.0237 5.80143L10.8365 8.90577L11.5887 13.2914L7.64931 11.2197L3.70993 13.2914L4.46211 8.90577L1.2749 5.80143L5.67962 5.15762L7.64931 1.16724Z" fill="var(--Color__Start)" stroke="var(--Color__Start)" stroke-width="1.27488" stroke-linecap="round" stroke-linejoin="round" />
                                                    </svg>
                                                </span>
                                            </div>
                                            <span class="personInfoEstimation--Estimation">5,0</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="commentContentText">
                                    <p class="commentContentText--text">
                                        До переезда в загородный дом о приглашении специалистов по уборке даже не задумывалась. С пылью в квартире спокойно справлялась сама. Но тут мы с мужем купили коттедж и решили переехать на постоянное место жительства. <br>
                                        Первое время было нормально, радовались природе, новым впечатлениям. Раскачивали сельскую жизнь, так сказать. Но в части уборки становилось все сложнее и сложнее. Дом 250 квадратов в несколько этажей. А еще и придомовая территория с мелкими постройками, бассейном. <br>
                                        В общем, в какой-то момент стала посматривать в сторону клининговых организаций. Интересовалась у друзей и в сообществах в интернете, как и что. Какую кто компанию может посоветовать. <br>
                                        Так, опытно-исследовательским путем вышла на Domoclean. Муж позвонил, пообщался, узнал про условия работы.стоимость и договорился о дате. К тому моменту я уже была готова на все. Лето же, а я только и бегаю по этажам с тряпками. <br>
                                        И, о счастье, через неделю команда бравых ребят была в нашем коттедже. <br>
                                        Мы взяли уборку по максимуму, вплоть до стен в пристройках и хозблоках. Специалисты использовали профессиональные средства в работе, вроде даже не аллергенные, как мне сказали. Использовали современное оборудование - пылесосы, пылесборники. <br>
                                        В общем, после их трудов коттедж стал белым, как снег, со свежайшим воздухом. Вычистили все щели, заглянули в самые темные углы. Отмыли сантехнику. Даже с бассейном разобрались. Не то, чтобы это все было сильно загрязнено, но налет уже был. <br>
                                        После такого результата я решила – хватит! Сама даже пытаться не буду повторить такое. Только мастера из "Домоклин" спасут меня от рутины. По цене, качеству и добросовестности – лучшие. Рада, что нашла такую компанию.
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>
                <section class="questions">
                    <div class="questionsWrapper">
                        <h2 class="questionsTitle">Вопросы и ответы</h2>

                        <ul class="questionsList">
                            <li class="questionsItems">
                                <button class="itemBtn">Сколько мне это будет стоить?</button>
                                <div class="itemInner">
                                    <p>
                                        Цена определяется с учетом площади. Есть зависимость от расположения. Ориентировочный расчет вы сможете получить, если заполните форму на сайте или свяжетесь с нами.
                                    </p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Входит ли мытье окон в стандартную услугу?</button>
                                <div class="itemInner">
                                    <p>
                                        Нет. Однако вы можете заказать это отдельно. При необходимости обеспечим чистку защитных конструкций.
                                    </p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Будут ли вымыты зеркала?</button>
                                <div class="itemInner">
                                    <p>Да. Зеркала и стеклянные поверхности мы в обязательном порядке моем. Все применяемые нами составы являются безопасными. Мы закупаем их у производителей с именем, тщательно тестируем перед использованием. Они безвредны, не провоцируют аллергических реакций, являются экологически чистыми</p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Справляетесь ли вы со сложными загрязнениями?</button>
                                <div class="itemInner">
                                    <p>Разумеется. В процессе клининга уборщики избавятся от пыли, известкового налета, ржавчины, засохшего жира. Все поверхности станут исключительно чистыми. Генеральная уборка проводится нами с помощью пароочистителя Karcher. Благодаря ему мы справляемся даже с устойчивыми загрязнениями в труднодоступных местах</p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Сколько займет работа?</button>
                                <div class="itemInner">
                                    <p>Работа в среднем занимает 6-8 часов. Это зависит от площади дома, от загрязненности, иных факторов. Если будет необходимо провести дополнительные услуги вроде мытья окон, время на уборку увеличится.</p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Сколько работников будет?</button>
                                <div class="itemInner">
                                    <p>В среднем клининг занимает от 2-8 часов. При определении точного времени мы будем руководствоваться сложностью предстоящих работ, площадью помещений и особенностями заказанной услуги</p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Вы моете бытовую технику и внутри, и снаружи?</button>
                                <div class="itemInner">
                                    <p>Мы моем бытовую технику только снаружи. Мойка внутри-это доп услуга</p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Потолки и стены будут помыты?</button>
                                <div class="itemInner">
                                    <p>Нет. Для того чтобы не повредить поверхность стен и потолков, мы используем только сухое обеспыливание</p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Я должен (должна) присутствовать?</button>
                                <div class="itemInner">
                                    <p>Решать вам. Отсутствие или присутствие никак не влияют на качество работ. Объясняется это в первую очередь тем, что оплата осуществляется по факту их завершения, а не до. Поэтому уборщики заинтересованы в результате и качестве</p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Убираете ли вы за собой мусор?</button>
                                <div class="itemInner">
                                    <p>Естественно. После уборки помещений в коттедже наши клинеры заберут с собой отходы, осуществят при необходимости обеззараживание</p>
                                </div>
                            </li>
                            <li class="questionsItems">
                                <button class="itemBtn">Возможна ли уборка за чертой Казани(в пригороде)?</button>
                                <div class="itemInner">
                                    <p>Да, конечно. Доплата выезда за город составит +1000 рублей</p>
                                </div>
                            </li>
                        </ul>
                    </div>
                </section>
            </div>
        </div>
    </div>
</main>

<?php get_footer(); ?>